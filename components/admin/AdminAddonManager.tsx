  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
        <Layers size={16} className="text-[#A507FA]" />
          ...
          return (
            <div
              ...
              className={`p-3 rounded-2xl border cursor-pointer transition-all flex items-center justify-between text-xs ${
                isSelected
                  ? "bg-[#FAF2FF] dark:bg-[#1A032A] border-[#A507FA] text-[#1A1C1C] dark:text-white shadow-sm"
            ...
            >
              <div className="flex items-center gap-2 overflow-hidden">
                {isSelected ? (
                  <CheckSquare size={16} className="text-[#A507FA] flex-shrink-0" />
                ) : (
                  <Square size={16} className="text-gray-300 dark:text-gray-700 flex-shrink-0" />
                )}
                <span className="font-bold truncate">{addon.name}</span>
              </div>
              <span className="font-extrabold text-[#A507FA] ml-2 flex-shrink-0">+${addon.price}</span>
            </div>
          );
        })}
      </div>
